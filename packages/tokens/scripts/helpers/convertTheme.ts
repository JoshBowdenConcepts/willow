import { Theme, ConvertedTheme, Qualifiers, Token } from "../../index";
...
export function convertTheme(theme: Theme): ConvertedTheme {
...
const processQualifiers = (data: ConvertedTheme): ConvertedTheme => {
  ...
  data.forEach((group) => {
    ...
    if (updatedQualifiers.length === 1 && updatedQualifiers[0] === "light") {
      updatedQualifiers = [];
    }
    ...
    if (updatedQualifiers.length > 1) {
      ...
      } else if (updatedQualifiers[updatedQualifiers.length - 1] === "light") {
